...
func (delivery *MemberDelivery) DeleteMember(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	err := delivery.memberService.Delete(id)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(helper.FailedResponse("Failed to left club: " + err.Error()))
		return
	}

	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(helper.SuccessResponse("Success left club"))
}
